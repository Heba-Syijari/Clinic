...
export default function EditUnit({ open, setOpen, id }) {
  ...
  const Edit = async () => {
    let selected =
      UnitSelector[UnitSelector.findIndex((UnitItem) => UnitItem.id === id)];
    let formdata = new FormData();
    formdata.append("id", selected.id);
    formdata.append("test_unit", document.getElementById("UnitName").value);
    ...
    await axios
      ...
      .then((response) => {
        let content = UnitSelector.map((item) => {
          return item.id == id
            ? { id: id, unit: document.getElementById("UnitName").value }
            : item;
        });
